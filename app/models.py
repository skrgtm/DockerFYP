...
class UserAccount(UserMixin, db.Model):
    ...
    profile_picture = db.Column(
        db.String(255), nullable=True, default='../static/images/acc.jpg')

    roles = db.relationship("Role", secondary=user_role,
                            backref=db.backref("users", lazy="dynamic"))
    # bookings = db.relationship("Booking", backref = 'Users')

# Membership model to store information about membership types
    ...
    def __init__(self, User, Email, Password, Mobile, Membership_Type=None, Price=None):
        self.User = User
        self.Email = Email
...
class Sessions(db.Model):
    ...
    facility = db.relationship(
        "Facility", backref=db.backref("sessions", lazy="dynamic"))
    activities = db.relationship(
        "Activity", secondary=session_activity_association, backref=db.backref("sessions", lazy="dynamic"))
...
class Activity(db.Model):
    ...
    facility = db.relationship(
        "Facility", backref=db.backref("activities", lazy="dynamic"))
...
class Booking(db.Model):
    ...
    activity = db.relationship(
        "Activity", backref=db.backref("bookings", lazy="dynamic"))

    user = db.relationship(
        "UserAccount", backref=db.backref("bookings", lazy="dynamic"))
    session = db.relationship(
        "Sessions", backref=db.backref("bookings", lazy="dynamic"))

    # New column for receipt_id foreign key
    receipt_id = db.Column(db.Integer, db.ForeignKey('receipt.id'))
    receipt = db.relationship("Receipt", backref=db.backref("bookings", lazy="dynamic"), foreign_keys=[
                              receipt_id])  # Update relationship attribute for Receipt
...
class Membership(db.Model):
    ...
    def __init__(self, name, price, interval, currency):
        self.name = name
        self.price = price
